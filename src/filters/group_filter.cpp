...
void GroupFilterElement::AddTypes_()
{
...
GroupFilter::GroupFilter()
{
    ...
    current_filter_type = FilterType::kGroup;
}
...
void GroupFilter::Identify_(Dynamic::Var& filter)
{
    ...
    if(filter_json->get("contents").isEmpty() || !filter_json->get("contents").isArray())
        throw std::runtime_error("content in GroupFilter::Identify_() is wrong");
    ...
    for(std::size_t a = 0; a < contents_array->size(); a++)
    {
        ...
        if(!contents_array->isObject(a))
            throw std::runtime_error("contents_array[" + std::to_string(a) + "] is not an object in GroupFilter::Identify_()");
...
void GroupFilter::Incorporate_(VectorString& tmp_query, RowValueFormatterList& query_parameters)
{
    ...
    for(auto it = filter_elements_.begin(); it != filter_elements_.end(); it++)
    {
        ...
        switch(it->get_type())
        {
            case GroupFilterElement::Type::kAsc: tmp_query.push_back("ASC"); break;
            case GroupFilterElement::Type::kDesc: tmp_query.push_back("DESC"); break;
        }
    }
}
