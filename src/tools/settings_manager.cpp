...
void SettingsManager::ReadFunctions_()
{
    ...
    {
        std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML001." << std::endl;
        return;
    }
    ...
    for(YAML::const_iterator it = config["functions"].begin(); it != config["functions"].end(); ++it)
    {
        ...
        {
            std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML002." << std::endl;
            return;
        }
        ...
        {
            std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML003." << std::endl;
            return;
        }
        ...
        {
            std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML004." << std::endl;
            return;
        }
        ...
        {
            std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML005." << std::endl;
            return;
        }
        ...
        {
            std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML006." << std::endl;
            return;
        }
        ...
        for(YAML::const_iterator it2 = actions.begin(); it2 != actions.end(); ++it2)
        {
            ...
            {
                std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML007." << std::endl;
                return;
            }
            if (!it2->second["sqlCode"] || !it2->second["sqlCode"].IsScalar())
            {
                std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML008." << std::endl;
                return;
            }
            if (!it2->second["final"] || !it2->second["final"].IsScalar())
            {
                std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML009." << std::endl;
                return;
            }
            ...
            for(YAML::const_iterator it3 = parameters.begin(); it3 != parameters.end(); ++it3)
            {
                ...
                {
                    std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML010." << std::endl;
                    return;
                }
                if (!it3->second["value"])
                {
                    std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML011." << std::endl;
                    return;
                }
                if (!it3->second["editable"] || !it3->second["editable"].IsScalar())
                {
                    std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML012." << std::endl;
                    return;
                }
                ...
                if(parameter_type == "conditional")
                {
                    ...
                    {
                        std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML013." << std::endl;
                        return;
                    }
                    if(!parameter_value["column"] || !parameter_value["column"].IsScalar())
                    {
                        std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML014." << std::endl;
                        return;
                    }
                    if(!parameter_value["action_results"] || !parameter_value["action_results"].IsScalar())
                    {
                        std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML015." << std::endl;
                        return;
                    }
                    ...
                    {
                        std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML016." << std::endl;
                        return;
                    }
                ...
                else if(parameter_type == "field")
                {
                    ...
                    {
                        std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML017." << std::endl;
                        return;
                    }
                }
                else
            ...
            for(YAML::const_iterator it4 = conditions.begin(); it4 != conditions.end(); ++it4)
            {
                ...
                {
                    std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML018." << std::endl;
                    return;
                }
                if (!it4->second["value"])
                {
                    std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML019." << std::endl;
                    return;
                }
                if (!it4->second["conditionalField"]["row"] || !it4->second["conditionalField"]["row"].IsScalar())
                {
                    std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML020." << std::endl;
                    return;
                }
                if (!it4->second["conditionalField"]["column"] || !it4->second["conditionalField"]["column"].IsScalar())
                {
                    std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML021." << std::endl;
                    return;
                }
                ...
                {
                    std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML022." << std::endl;
                    return;
                }
                ...
                if(condition_type == "List")
                {
                    ...
                    for(YAML::const_iterator it5 = values.begin(); it5 != values.end(); ++it5)
                    {
                        ...
                        {
                            std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML023." << std::endl;
                            return;
                        }
                    }
                }
                else
                {
                    ...
                    {
                        std::cout << "- Error on backend_handler.cpp on AddRoutes_(): The functions.yaml file is malformed. ERRYML024." << std::endl;
                        return;
                    }
                }
